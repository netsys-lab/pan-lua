...
var (
	statsName                        = C.CString("stats")
	TracerForConnection              = C.CString("TracerForConnection")
	StartedConnection                = C.CString("StartedConnection")
	NegotiatedVersion                = C.CString("NegotiatedVersion")
	ClosedConnection                 = C.CString("ClosedConnection")
	SentTransportParameters          = C.CString("SentTransportParameters")
	ReceivedTransportParameters      = C.CString("ReceivedTransportParameters")
	RestoredTransportParameters      = C.CString("RestoredTransportParameters")
	SentPacket                       = C.CString("SentPacket")
	ReceivedVersionNegotiationPacket = C.CString("ReceivedVersionNegotiationPacket")
	ReceivedRetry                    = C.CString("ReceivedRetry")
	ReceivedPacket                   = C.CString("ReceivedPacket")
	BufferedPacket                   = C.CString("BufferedPacket")
	DroppedPacket                    = C.CString("DroppedPacket")
	UpdatedMetrics                   = C.CString("UpdatedMetrics")
	AcknowledgedPacket               = C.CString("AcknowledgedPacket")
	LostPacket                       = C.CString("LostPacket")
	UpdatedCongestionState           = C.CString("UpdatedCongestionState")
	UpdatedPTOCount                  = C.CString("UpdatedPTOCount")
	UpdatedKeyFromTLS                = C.CString("UpdatedKeyFromTLS")
	UpdatedKey                       = C.CString("UpdatedKey")
	DroppedEncryptionLevel           = C.CString("DroppedEncryptionLevel")
	DroppedKey                       = C.CString("DroppedKey")
	SetLossTimer                     = C.CString("SetLossTimer")
	...
	LatestRTT                        = C.CString("LatestRTT")
	MaxAckDelay                      = C.CString("MaxAckDelay")
	MeanDeviation                    = C.CString("MeanDeviation")
	MinRTT                           = C.CString("MinRTT")
	PTO                              = C.CString("PTO")
	SmoothedRTT                      = C.CString("SmoothedRTT")
)
...
func (s *Stats) TracerForConnection(tracer_id uint64, p logging.Perspective, odcid logging.ConnectionID) error {
	...
	s.Lock()
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, TracerForConnection)
	s.Remove(-2)
...
func (s *Stats) StartedConnection(local, remote *pan.UDPAddr, srcConnID, destConnID logging.ConnectionID) error {
	...
	s.GetCGlobal(statsName)
	s.GetCField(-1, StartedConnection)
	s.Remove(-2)
...
func (s *Stats) NegotiatedVersion(local, remote *pan.UDPAddr, chosen logging.VersionNumber, clientVersions, serverVersions []logging.VersionNumber) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, NegotiatedVersion)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
...
func (s *Stats) ClosedConnection(local, remote *pan.UDPAddr, err error) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, ClosedConnection)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
...
func (s *Stats) SentTransportParameters(local, remote *pan.UDPAddr, parameters *logging.TransportParameters) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, SentTransportParameters)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
...
func (s *Stats) ReceivedTransportParameters(local, remote *pan.UDPAddr, parameters *logging.TransportParameters) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, ReceivedTransportParameters)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushTable(new_lua_parameters(parameters))
...
func (s *Stats) RestoredTransportParameters(local, remote *pan.UDPAddr, parameters *logging.TransportParameters) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, RestoredTransportParameters)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushTable(new_lua_parameters(parameters))
...
func (s *Stats) ReceivedVersionNegotiationPacket(local, remote *pan.UDPAddr, hdr *logging.Header, versions []logging.VersionNumber) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, ReceivedVersionNegotiationPacket)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushTable(vs)
...
func (s *Stats) AcknowledgedPacket(local, remote *pan.UDPAddr, level logging.EncryptionLevel, num logging.PacketNumber) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, AcknowledgedPacket)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
...
func (s *Stats) UpdatedCongestionState(local, remote *pan.UDPAddr, state logging.CongestionState) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, UpdatedCongestionState)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
...
func (s *Stats) UpdatedKeyFromTLS(local, remote *pan.UDPAddr, level logging.EncryptionLevel, p logging.Perspective) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, UpdatedKeyFromTLS)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushInteger(int64(level))
...
func (s *Stats) DroppedEncryptionLevel(local, remote *pan.UDPAddr, level logging.EncryptionLevel) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, DroppedEncryptionLevel)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushInteger(int64(level))
...
func (s *Stats) SetLossTimer(local, remote *pan.UDPAddr, ttype logging.TimerType, level logging.EncryptionLevel, t time.Time) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, SetLossTimer)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushInteger(int64(ttype))
	s.PushInteger(int64(level))
	s.PushInteger(t.Unix())
	err := s.Call(5, 0)
	if err != nil {
		log.Println(err)
	}
...
func (s *Stats) LossTimerExpired(local, remote *pan.UDPAddr, ttype logging.TimerType, level logging.EncryptionLevel) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, LossTimerExpired)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	s.PushInteger(int64(ttype))
...
func (s *Stats) LossTimerCanceled(local, remote *pan.UDPAddr) error {
	...
	defer s.Unlock()
	s.GetCGlobal(statsName)
	s.GetCField(-1, LossTimerCanceled)
	s.Remove(-2)
	s.pushAsLuaString(local)
	s.pushAsLuaString(remote)
	err := s.Call(2, 0)
